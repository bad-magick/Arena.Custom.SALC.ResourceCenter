...
/****** Object:  StoredProcedure [dbo].[cust_sp_salc_rc_rprt_helpsummary_byday]    Script Date: 04/13/2012 13:42:54 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROC [dbo].[cust_sp_salc_rc_rprt_helpsummary_byday]
/*
-- =============================================
-- Author:		Den Boice
...
SELECT
	...
	CASE
		WHEN @StartDate < [cust_salc_rc_help].[date]
			THEN [cust_salc_rc_help].[date]
		ELSE
			@StartDate
	END AS [StartDate],
	CASE
		WHEN @EndDate > [cust_salc_rc_help].[completed_on]
			THEN [cust_salc_rc_help].[completed_on]
		ELSE
			@EndDate
	END AS [EndDate]
INTO #clients
...
WHERE 
	(CASE
		WHEN @Direction = 0 AND
			...
			AND [cust_salc_rc_help].sub_type = @AssistanceType
			THEN 1
...
SELECT DISTINCT
	...
	CASE
		WHEN @EndDate > [cust_salc_rc_help].[completed_on]
			THEN CASE 
				WHEN CONVERT(date, '1/1/1901') < [cust_salc_rc_help].[completed_on]
					THEN [cust_salc_rc_help].[completed_on]
				ELSE @EndDate 
			END
		ELSE
...
;WITH Calendar AS
	...
	SELECT
		Calendar.[Date] AS [Day],
		(
			SELECT COUNT(*) 
			FROM #helps 
			WHERE Calendar.[Date] BETWEEN #helps.[StartDate] AND #helps.[EndDate]
		) AS [Total],
		(
			SELECT SUM(Children)
			FROM #helps
	...
	FROM Calendar
		LEFT JOIN #helps
			ON Calendar.[Date] BETWEEN #helps.[StartDate] AND #helps.[EndDate]
